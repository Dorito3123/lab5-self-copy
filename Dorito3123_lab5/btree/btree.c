...
static BTreeNode* makenode(int leaf) {
...
static int childIdx(BTreeNode* n, const char* key) {
    ...
    while (i < n->n && strcmp(key, n->keys[i]) > 0) {
        i++;
    }
...
static void splitChild(BTreeNode* x, int i, BTreeNode* y) {
    int t = BTREE_T;
    BTreeNode* z = makenode(y->is_leaf);
    z->n = t - 1;
    ...
    if (!y->is_leaf) for (int j = 0; j < t; j++) {
...
static void insNonFull(BTreeNode* n, const char* key, int doc_id, const char* title) {
    ...
    if (idx >= 0) { 
        appendPosting(n->postings[idx], doc_id, title);
        return; 
    }
...
static int appendIfExists(BTreeNode* n, const char* key, int doc_id, const char* title) {
    ...
    int i = findKey(n, key);
    if (i >= 0) {
        appendPosting(n->postings[i], doc_id, title); return 1;
    }
...
static void walkNode(BTreeNode* n, void (*visit)(const char* key, Vector* postings, void* ctx), void* ctx) {
    if (!n) return;

    for (int i = 0; i < n->n; i++) {
        if (!n->is_leaf) {
            walkNode(n->children[i], visit, ctx);
        }
...
Vector* btreeSearch(const BTree* t, const char* key) {
    return findIn(t->root, key);
}

void btreeTraverse(const BTree* t, void (*visit)(const char* key, Vector* postings, void* ctx), void* ctx) {
