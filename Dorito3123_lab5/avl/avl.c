...
static AVLNode* ins(AVLNode* n, const char* key, int doc_id, const char* title, int* added) {
    if (!n) { *added = 1; return makeNode(key, doc_id, title); }
    int c = strcmp(key, n->key);
    if      (c < 0) n->left  = ins(n->left,  key, doc_id, title, added);
    else if (c > 0) n->right = ins(n->right, key, doc_id, title, added);
    else { appendPosting(n->postings, doc_id, title); return n; }

    upd(n);
    int b = bf(n);
    if (b >  1 && strcmp(key, n->left->key)  < 0) return rr(n);
    if (b < -1 && strcmp(key, n->right->key) > 0) return rl(n);
    if (b >  1 && strcmp(key, n->left->key)  > 0) { n->left  = rl(n->left);  return rr(n); }
    if (b < -1 && strcmp(key, n->right->key) < 0) { n->right = rr(n->right); return rl(n); }
    return n;
}
...
static void drop(AVLNode* n) {
    if (!n) return;
    drop(n->left); drop(n->right);
    free(n->key); vectorFree(n->postings); free(n);
}
...
void freeAVLTree(AVLTree* t) { if (t) { drop(t->root); free(t); } }
...
void avlTraverse(const AVLTree* t,
                 void (*visit)(const char* key, Vector* postings, void* ctx), void* ctx) {
