    ...
    while (z->parent->color == RB_RED) {
        if (z->parent == z->parent->parent->left) {
            ...
            if (u->color == RB_RED) {
                ...
                if (z == z->parent->right) { z = z->parent; rotL(t, z); }
            ...
            if (u->color == RB_RED) {
                ...
                if (z == z->parent->left) { z = z->parent; rotR(t, z); }
                z->parent->color         = RB_BLACK;
                z->parent->parent->color = RB_RED;
...
static void dropNodes(RBTree* t, RBNode* n) {
    if (n == t->nil) return;
    dropNodes(t, n->left); dropNodes(t, n->right);
...
RBTree* createRBTree(void) {
    ...
    nil->color = RB_BLACK; nil->key = NULL; nil->postings = NULL;
...
void freeRBTree(RBTree* t) {
    if (!t) return;
    dropNodes(t, t->root); free(t->nil); free(t);
}
...
void rbInsert(RBTree* t, const char* key, int doc_id, const char* title) {
    ...
    while (cur != t->nil) {
        ...
        if (c == 0) { appendPosting(cur->postings, doc_id, title); return; }
        cur = c < 0 ? cur->left : cur->right;
    }

    RBNode* z = mknode(t, key, doc_id, title);
    RBNode* y = t->nil, *x = t->root;
    while (x != t->nil) { y = x; x = strcmp(z->key, x->key) < 0 ? x->left : x->right; }
    z->parent = y;
    if (y == t->nil)                    t->root  = z;
...
Vector* rbSearch(const RBTree* t, const char* key) {
    ...
    while (cur != t->nil) {
        int c = strcmp(key, cur->key);
        if (c == 0) return cur->postings;
        cur = c < 0 ? cur->left : cur->right;
    }
    return NULL;
}

static void walkNodes(RBTree* t, RBNode* n,
